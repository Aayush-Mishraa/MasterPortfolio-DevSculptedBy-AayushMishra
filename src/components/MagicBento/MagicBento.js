...
const MagicBento = ({
  ...
  const isDarkTheme = 
    ...
    currentTheme.id?.includes('Dark') ||
    ['cyberpunk', 'oceanDepth', 'midnightPurple', 'neonGreen', 'darkOrange', 'crimsonDark', 'galaxy', 'nordDark', 'dracula', 'monochrome', 'amoled', 'midnight', 'neon', 'materialDark'].includes(currentTheme.id);
  ...
  useEffect(() => {
    if (gridRef.current) {
      gridRef.current.setAttribute('data-theme', currentTheme.id || 'light');
    }
    document.documentElement.setAttribute('data-theme', currentTheme.id || 'light');
  }, [currentTheme]);

  return (
    <div className="magic-bento-container" data-theme={currentTheme.id}>
